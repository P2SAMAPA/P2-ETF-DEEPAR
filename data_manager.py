"""
Data loading and preprocessing for DeepAR engine.
"""
...
def create_sequences(data: np.ndarray, context_len: int, pred_len: int):
    """Create sliding windows for training DeepAR."""
    X, y = [], []
    for i in range(len(data) - context_len - pred_len + 1):
        X.append(data[i:i+context_len])
        y.append(data[i+context_len:i+context_len+pred_len])
